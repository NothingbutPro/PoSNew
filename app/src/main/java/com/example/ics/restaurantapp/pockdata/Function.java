package com.example.ics.restaurantapp.pockdata;
			...
			if (inBuffer != null && inBuffer.length > 2) {
				...
				if (inBuffer[index] == PostDefine.ACK) {
					...
					if (inBuffer.length > index) {
						...
						{
							postMsg.setReturnType(PostMessage.ReturnType.PATH);
							...
							if (msgLen == len + 8) {
								...
								postMsg.setReturnType(PostMessage.ReturnType.Integrity);
							}
						}
					} else {
						postMsg.setReturnType(PostMessage.ReturnType.NONE);
					}
				}
			}
		} catch (Exception ex) {
